...
var rotatingPanel = function () {
    ...
    if (panels.length > 0) {
        function StartTimer(position) {
            $(document).everyTime('10s', 'rotating-panel', function () { rotatingPanel.rotate(position); }, 0);
        }
...
/* Rotating Tips*/
var rotatingTips = function () {

    var panels = [];

    $('.rotating-tip').each(function (i) {
        panels.push($(this));
    })

    if (panels.length > 0) {
        function StartTimer(position) {
            $(document).everyTime('10s', 'rotating-tip', function () { rotatingTips.rotate(position); }, 0);
        }

        function StopTimer() {
            $(document).stopTime('rotating-tip');
        }
